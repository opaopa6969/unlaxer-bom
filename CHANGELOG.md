...
>   （旧パスが 404 になっていた）。月次更新スクリプトの動作に直結する修正。
>   テスト 111 件 green。
...
>   実測（ダミー 13 か月）: 行数 1,630,279 → 127,015（13 分の 1）、
>   DB 494MB → 30MB（16 分の 1）、初回構築 31,746ms → 1,526ms（21 倍速い）。
>   223 か月分でも行数は約 14.1 万でほぼ線形に増えない見込み。
>   突き合わせテスト 12 件でメモリ版と同じ結果を確認済み（全 97 件 green）。
